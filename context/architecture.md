...
| Auth      | Clerk                       | User indentity and route protection          |
...
- `app/api` — Authenticated request handlers: input validation, ownership checks, task 
triggering, and persistence.
- `triger` — Long-running background jobs: AI design generation and spec generation.
- `lib` — Shared ingreastructure: Prisma client, access control helpers, and utilities.
- `components` — UI composition: canvas surfaces, sidebars. dialogs, and interative elements.
- `prisma` — Database shcema and generated client outout.
- `data` — Legacy local dirtectory. Not used for new artifacts.

## Storage Model

- **Database**: metadata, ownership, relationships, and task run records.
- **Vercel Blob**: generated artifacts — canvas snapshots at `canvas/{projectId}.json` and specs at 
`spects/{projectId}/{spectId}.md`.
...
- The blob URL is sotred in database (`canvasJsonPath`, `filtePath`) as the reference to the artifact.

## Auth and Collaboration Model

- Every project has a single owner (Clerk user ID).
- Projects can include additional can include additional collaborators.
- Only authenticated userscan access protected routes.
...
- Templates do not require a separate database record;  they resolved by template ID not 
at import time.
...
- Execution: durable bakcground task via Trigger.dev.
